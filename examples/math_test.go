...
func assertMathAddOneInvocation(t *testing.T, mocker *MathMocker, ctx context.Context, a, b, sum int, err error) {
	...
	assert.Equal(t, ctx, invoc.Inputs.Arg1, "wrong context")
	assert.Equal(t, a, invoc.Inputs.Arg2, "wrong first integer")
	assert.Equal(t, b, invoc.Inputs.Arg3, "wrong second integer")
	assert.Equal(t, sum, invoc.Outputs.Out1, "wrong first returns")
	assert.Equal(t, err, invoc.Outputs.Out2, "wrong second returns")
}
...
func assertMathSubtractOneInvocation(t *testing.T, mocker *MathMocker, ctx context.Context, a, b, result int, err error) {
	...
	assert.Equal(t, a, invoc.Inputs.Arg2, "wrong first integer")
	assert.Equal(t, b, invoc.Inputs.Arg3, "wrong second integer")
	assert.Equal(t, result, invoc.Outputs.Out1, "wrong first returns")
	assert.Equal(t, err, invoc.Outputs.Out2, "wrong second returns")
}
