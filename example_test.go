...
func TestAddFuncMocker_MockReturnValueOnce(t *testing.T) {
	mocker, f := NewMockedAddFunc()
	mocker.MockReturnValueOnce(20, errors.New("error_test"))
	sum, err := f(context.Background(), 10, 20)
	assert.NotNil(t, err)
...
func TestAddFuncMocker_MockReturnValueForever(t *testing.T) {
	mocker, f := NewMockedAddFunc()
	mocker.MockReturnValueForever(20, errors.New("error_test"))
	for i := 0; i < 100; i++ {
		sum, err := f(context.Background(), 10, 20)
		assert.NotNil(t, err)
		assert.Equal(t, "error_test", err.Error())
...
func TestAddFuncMocker_MockReturnValue(t *testing.T) {
	mocker, f := NewMockedAddFunc()
	mocker.MockReturnValue(10, 20, errors.New("error_test"))
	for i := 0; i < 10; i++ {
		sum, err := f(context.Background(), 10, 20)
		assert.NotNil(t, err)
		assert.Equal(t, "error_test", err.Error())
...
func TestAddFuncMocker_MockFuncOnce(t *testing.T) {
	mocker, f := NewMockedAddFunc()
	mocker.MockFuncOnce(func(p1 context.Context, p2 int, p3 int) (i int, e error) {
		return p2 + p3, nil
	})
	sum, err := f(context.Background(), 10, 20)
	assert.Nil(t, err)
...
func TestAddFuncMocker_MockFuncForever(t *testing.T) {
	mocker, f := NewMockedAddFunc()
	mocker.MockFuncForever(func(p1 context.Context, p2 int, p3 int) (i int, e error) {
		return p2 + p3, nil
	})
...
func TestAddFuncMocker_MockFunc(t *testing.T) {
	mocker, f := NewMockedAddFunc()
	mocker.MockFunc(10, func(p1 context.Context, p2 int, p3 int) (i int, e error) {
		return p2 + p3, nil
	})
...
func TestAddFuncMocker_Invocations(t *testing.T) {
	mocker, f := NewMockedAddFunc()
	mocker.MockReturnValueForever(19, nil)
	...
	for _, iv := range invocations {
		assert.Equal(t, ctx, iv.Parameter.p1)
		assert.Equal(t, 10, iv.Parameter.p2)
		assert.Equal(t, 20, iv.Parameter.p3)
...
func TestAddFuncMocker_TakeOneInvocation(t *testing.T) {
	...
	mocker.MockReturnValueForever(19, err)
	for i := 0; i < 10; i++ {
		f(ctx, 10, 20)
	}
